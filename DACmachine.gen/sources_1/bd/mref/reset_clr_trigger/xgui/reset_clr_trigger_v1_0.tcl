# Definitional proc to organize widgets for parameters.
proc init_gui { IPINST } {
  ipgui::add_param $IPINST -name "Component_Name"
  #Adding Page
  set Page_0 [ipgui::add_page $IPINST -name "Page 0"]
  ipgui::add_param $IPINST -name "CLR_CLKS" -parent ${Page_0}
  ipgui::add_param $IPINST -name "RESET_CLKS" -parent ${Page_0}


}

proc update_PARAM_VALUE.CLR_CLKS { PARAM_VALUE.CLR_CLKS } {
	# Procedure called to update CLR_CLKS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.CLR_CLKS { PARAM_VALUE.CLR_CLKS } {
	# Procedure called to validate CLR_CLKS
	return true
}

proc update_PARAM_VALUE.RESET_CLKS { PARAM_VALUE.RESET_CLKS } {
	# Procedure called to update RESET_CLKS when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.RESET_CLKS { PARAM_VALUE.RESET_CLKS } {
	# Procedure called to validate RESET_CLKS
	return true
}


proc update_MODELPARAM_VALUE.RESET_CLKS { MODELPARAM_VALUE.RESET_CLKS PARAM_VALUE.RESET_CLKS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.RESET_CLKS}] ${MODELPARAM_VALUE.RESET_CLKS}
}

proc update_MODELPARAM_VALUE.CLR_CLKS { MODELPARAM_VALUE.CLR_CLKS PARAM_VALUE.CLR_CLKS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.CLR_CLKS}] ${MODELPARAM_VALUE.CLR_CLKS}
}

